...
export default function BottomActions() {
  ...
  const { id, isGeneratingPDF, createQuotation, formData } = useQuotation();
    ...
    <div className="w-full flex items-center justify-center space-x-4 pt-6 border-t border-gray-200">
      ...
      {id ?
        location.pathname.startsWith('/quotations/edit') ?
          ...
          >
            <Send className="w-4 h-4" />
            <span>{isGeneratingPDF ? "Creating Duplicate..." : `Create Duplicate ${formData.send_immediately? "& Send" : ""}`}</span>
          </button>
        :
        <button
          onClick={createQuotation}
